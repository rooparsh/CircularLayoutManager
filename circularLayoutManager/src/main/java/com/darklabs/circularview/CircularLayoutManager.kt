...
class CircularLayoutManager : LinearLayoutManager {
    ...
    private var mScalingFactor: Float = 0f
    ...
    constructor(context: Context,
        ...
        with(context.obtainStyledAttributes(attr, R.styleable.RecyclerView, defStyleAttr,
                defStyleRes)) {

            super.setOrientation(
                    createOrientationFromAttr(getInt(R.styleable.RecyclerView_orientation,
                            Orientation.VERTICAL)))

            setGravity(createGravityFromAttr(getInt(R.styleable.RecyclerView_gravity,
                    Gravity.START)))
            ...
            setRotating(getBoolean(R.styleable.RecyclerView_rotating, false))
            setScalingFactor(getFloat(R.styleable.RecyclerView_scalingFactor, 0.000f))
            setScaling(getBoolean(R.styleable.RecyclerView_scaling, false))
            mCenter = Point()
            recycle()
        }
    }
    ...
    override fun onLayoutChildren(recycler: Recycler,
    ...
    private fun setChildOffsetsVertical(@Gravity gravity: Int,
        ...
        (0 until childCount).forEach { i ->
            ...
            setChildVerticalScaling(child)
            setChildRotationVertical(gravity, child, radius, center)
        }
    }

    private fun setChildHorizontalScaling(child: View) {
        if (mIsScaling.not()) {
            return
        }
        val halfWidth = child.width * 0.5f
        val parentHalfWidth = width * 0.5f
        val x = child.x
        val rot = (parentHalfWidth - halfWidth - x) / 10000
        val scale = 1.0f - abs(rot) * mScalingFactor
    ...
    private fun setChildOffsetsHorizontal(@Gravity gravity: Int,
        ...
        (0 until childCount).forEach { i ->
            ...
            setChildHorizontalScaling(child)
            setChildRotationHorizontal(gravity, child, radius, center)
        }
    }

    private fun setChildVerticalScaling(child: View) {

        if (mIsScaling.not()) {
            return
        }

        val halfHeight = child.height * 0.5f
        val parentHalfHeight = height * 0.5f
        val y = child.y
        val rot = (parentHalfHeight - halfHeight - y) / 10000
        val scale = 1.0f - abs(rot) * (mScalingFactor)
    ...
    fun setScaling(isScaling: Boolean) {
        this.mIsScaling = isScaling
        requestLayout()
    }
